...
ui_print " ";
ui_print "mtd-utils Installer Script";
...
mkdir -p $bin;
cp -rf bin/* $bin;
chown 0:0 "$bin/flash_erase" "$bin/nanddump" "$bin/nandwrite";
chmod 755 "$bin/flash_erase" "$bin/nanddump" "$bin/nandwrite";
if [ "$magisk" ]; then
